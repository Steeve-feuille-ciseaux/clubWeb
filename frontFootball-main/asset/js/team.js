...
function addPlayer() {
    ...
    // Obtenir la position prédéfinie selon le poste
    const positionsForFormation = formationPositions[formation][position];
    const currentPlayersInPosition = players.filter(p => p.position === position).length;
    const defaultPosition = positionsForFormation[currentPlayersInPosition] || { x: 50, y: 50 };
